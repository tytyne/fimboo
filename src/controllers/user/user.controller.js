...
const { createUser,getUserByIdOrEmail,updateUser,getUserByUsernameOrEmail} = UserService;
const { hashPassword } = helper;
const { signedup,accountVerified,resend,userVerification,loggedin} = customMessage;
const{emailAssociate,thisAccountVerified,emailOrUsernameNotFound,accountNotVerified,emailOrPasswordNotFound,accountNotActivated}=errorMessage
...
/**
  ...
  //     const { formData, userRole } = req.body;
  //     const textPassword = formData.password;
  //     formData.password = hashPassword(textPassword);
  ...
  //       header: 'Confirm your email',
  //       messageHeader: `Hi, <strong>${user.firstname}!</strong>`,
  //       messageBody: `the Business owner {tytyne}created an account for you,to confirm your email, Click the following Button to confirm your email.`,
  //       optionLink: `${process.env.APP_URL}/api/${process.env.API_VERSION}/user/confirmation/${token}`,
  //       browserMessage:`If that doesn't work, copy and paste this link into your browser`,
  //       Button:true
  //     });
  ...
  //     });
  //     await mail.sendMail();
  //     addEmailToMailchimp(user.email,user.firstname,user.lastname)
