...
/**
  ...
  static async signup(req, res,next) {
    ...
    // try{
      ...
      const user = await createBusinessOwner(formData);
      console.log(user)
  ...
  static async confirmation(req,res,next){
    ...
    try{
      ...
      const user=await getBusinessOwnerByIdOrEmail(decoded.email)
      if(user.isVerified){
        ...
        await mail.sendMail();
        const userUpdated = await updateBusinessOwner(decoded)
  ...
  static async resend(req,res,next){
    try{
      ...
      const user= await getBusinessOwnerByIdOrEmail(email)
  ...
  static async login(req, res, next) {
    try {
     ...
     const  user = await getBusinessOwnerByUsernameOrEmail(email);
    ...
    static async resetPassword(req,res,next){
        ...
        try{
            ...
            const { token } = req.params;
            const decoded = jwtToken.verifyToken(token);
            const user= await getBusinessOwnerByEmail(decoded.email)
            const mail = new Mailer({
                to: `${user.username} <${user.email}>`,
